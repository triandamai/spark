...
abstract class BaseElementBuilder<T : BaseElementBuilder<T>>(
     ...
     * ```
     */
    fun <ITEM> reactivity.State<MutableList<ITEM>>.each(block: dom.builders.BaseElementBuilder.EachScope<ITEM, BaseElementBuilder<*>>.(ITEM) -> Unit) {
     ...
     * ```
     */
    fun <ITEM> reactivity.State<MutableList<ITEM>>.eachIndexed(block: dom.builders.BaseElementBuilder.EachScope<ITEM, BaseElementBuilder<*>>.(Int, ITEM) -> Unit) {
        this.value.forEachIndexed { index, item ->
            val scope = EachScope<ITEM, BaseElementBuilder<*>>(index, this@BaseElementBuilder)
            scope.block(index, item)
        }
