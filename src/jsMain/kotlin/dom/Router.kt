...
class Router(
    ...
    fun navigate(path: String): Boolean {
        ...
        console.log("Navigating from ${fromRoute?.path} to ${toRoute.path}")
        val isNavigated = navigationHook?.beforeNavigate(defaultFrom, toRoute, next) ?: true
        if (isNavigated) {
            return nextComponent.beforeNavigate(defaultFrom, toRoute, next)
        }
        return false
    }
